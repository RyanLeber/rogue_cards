...
export default defineEventHandler(async (event) => {
  ...
  if ( isGameSet() ) {
    ...
    try {
      ...
      const { rows: gameRows, rowCount: gameCount } = await client.sql`SELECT * FROM games WHERE game_token = ${gameToken.value}`;
  ...
  } else { return new Error('User in not logged in');}
});