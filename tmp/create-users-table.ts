...
const createTable7 = await sql`
CREATE TABLE IF NOT EXISTS game_relics (
  relic_id SERIAL PRIMARY KEY,
  relic_name VARCHAR(255),
  relic_description VARCHAR(255)
);
`

const createTable8 = await sql`
CREATE TABLE IF NOT EXISTS game_trump_cards (
  trump_id SERIAL PRIMARY KEY,
  trump_name VARCHAR(255),
  trump_description VARCHAR(255),
  energy INT
);
`


const createTable9 = await sql`
CREATE TABLE IF NOT EXISTS game_potions (
  potion_id SERIAL PRIMARY KEY,
  potion_name VARCHAR(255),
  potion_description VARCHAR(255),
  gold INT
);
`
...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    await client.sql`INSERT INTO game_relics (relic_name, relic_description) VALUES ()`
...
//        Index on user_id in sessions table
//        CREATE INDEX idx_sessions_user_id ON sessions(user_id)   
     
//        Index on game_token in games table, if not unique
//        CREATE INDEX idx_games_game_token ON games(game_token);

    // useCookie('sessionToken', { httpOnly: true, secure: true, path: '/', expires: expiresAt });
